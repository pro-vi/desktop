...
test('chatgpt-controller: durable reconciliation reaches a hard terminal with final diagnostics', async () => {
  const realNow = Date.now;
    ...
    async evaluate(js) {
      ...
      if (js.includes('publishedCaptureWindow')) {
        return {
          ...
          artifactInventory: { status: 'partial', reason: 'conversation_generation_active', items: [] }
        };
      }
      if (js.includes('fallbackMainText')) {
        return {
          ...
          isThinking: false,
          pageText: '',
          currentUrl: 'https://chatgpt.com/c/reconcile-timeout'
        };
      }
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
    async getUrl() { return 'https://chatgpt.com/c/reconcile-timeout'; },
    async sendKey() {},
    async insertText() {},
    async moveMouse() {},
    async mouseDown() {},
  ...
  try {
    await assert.rejects(
      ...
      (error) => {
        ...
        assert.equal(error?.data?.responseDebug?.hardDeadlineMs, 2_000);
        assert.equal(error?.data?.recovery?.status, 'partial');
        return true;
      }
    );
  } finally {
...
test('chatgpt-controller: hard reconciliation deadline bounds a hung response evaluation', async () => {
  ...
  const page = {
    ...
    async evaluate(js) {
      ...
      if (js.includes('fallbackMainText') && js.includes('imageCandidateCount')) {
        return await new Promise((resolve) => { settleHungEvaluation = resolve; });
      }
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
    async terminateEvaluation() {
      terminationCalls += 1;
      settleHungEvaluation?.(null);
      return true;
    },
    async getUrl() { return sent ? 'https://chatgpt.com/c/hung-response' : 'https://chatgpt.com/'; },
    async sendKey() {},
    async insertText() {},
    async moveMouse() {},
    ...
    async evaluate(js) {
      ...
      if (js.includes('fallbackMainText')) {
        return {
          ...
          isThinking: false,
          pageText: '',
          currentUrl: 'https://chatgpt.com/c/reconcile-recovered'
        };
      }
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
...
test('chatgpt-controller: final reconciliation rejects a baseline-only virtualized conversation', async () => {
  const realNow = Date.now;
    ...
    async evaluate(js) {
      ...
      if (js.includes('fallbackMainText')) {
        return {
          ...
          isThinking: false,
          pageText: '',
          currentUrl: 'https://chatgpt.com/c/baseline-only'
        };
      }
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
    async getUrl() { return 'https://chatgpt.com/c/baseline-only'; },
    async sendKey() {},
    async insertText() {},
    async moveMouse() {},
    async mouseDown() {},
  ...
  try {
    await assert.rejects(
      ...
      (error) => {
        assert.equal(error?.message, 'response_reconcile_timeout');
        assert.equal(error?.data?.recovery?.advanced, false);
        return true;
      }
    );
  } finally {
...
test('chatgpt-controller: already-selected supported Pro slider confirms without a trigger candidate', async () => {
    ...
    async evaluate(js) {
      ...
      if (js.includes('fallbackMainText')) {
        return {
          ...
          sendEnabled: true,
          sendFound: true,
          txt: 'Confirmed without a second slider click',
          count: 1,
          usedFallback: false,
          hasError: false,
          hasContinue: false,
          hasRegenerate: false,
          isThinking: false,
          pageText: 'Confirmed without a second slider click\nPro\nChatGPT can make mistakes. Check important info.'
        };
      }
      if (js.includes('const codes = Array.from')) return { codeBlocks: [] };
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
    async getUrl() { return 'https://chatgpt.com/c/already-pro'; },
    async sendKey() {},
    async insertText() {},
    async moveMouse() {},
    async mouseDown(x, y) { pointerDown.push({ x, y }); },
    async mouseUp() {},
    async setFileInputFiles() {}
  };
  const controller = new ChatGPTController({
  ...
  try {
    ...
    assert.equal(result.text, 'Confirmed without a second slider click');
  } finally {
    Date.now = realNow;
  }
  assert.equal(modeChecks, 1);
  assert.equal(pointerDown.some((event) => event.y === 74), false);
  assert.equal(pointerDown.some((event) => event.x > 300), true);
  const provenance = progress.find((patch) => patch.phase === 'mode_intent_confirmed')?.modeIntentProvenance;
  assert.equal(provenance?.evidenceKind, 'supported_power_slider');
  assert.equal(provenance?.powerMin, 0);
  assert.equal(provenance?.powerMax, 4);
  assert.equal(provenance?.powerIndex, 4);
...
test('chatgpt-controller: the composer slider label does not read as a downgraded mode', async () => {
  ...
  // control, not the mode that answered, so it must not trip the downgrade check.
  const realNow = Date.now;
  let fakeNow = 6_750_000;
  Date.now = () => {
    fakeNow += 1_000;
    return fakeNow;
  };
    ...
    async evaluate(js) {
      ...
      if (js.includes('fallbackMainText')) {
        return {
          ...
          hasRegenerate: false,
          isThinking: false,
          pageText: 'Reply with exactly one word: ok\nWorked for 9s\nok\nChatGPT can make mistakes. Check important info.\nThinking effort'
        };
      }
      if (js.includes('const codes = Array.from')) return { codeBlocks: [] };
      throw new Error(`unexpected_eval:${js.slice(0, 80)}`);
    },
    async getUrl() { return 'https://chatgpt.com/c/pro-slider-label'; },
    async sendKey() {},
    async insertText() {},
    async moveMouse() {},
    async mouseDown() {},
  ...
  try {
    ...
    assert.notEqual(result.meta?.modeUsed, 'thinking');
  } finally {
    Date.now = realNow;
  }
});
...
test('chatgpt-controller: readConversationText preserves the partial capture reason under a preview limit', async () => {
  const page = virtualizedConversationPage([
    { role: 'assistant', text: 'Reply without its prompt' }
  ]);
  const controller = new ChatGPTController({ page, selectors: {} });
  ...
  assert.equal(result.captureReason, 'leading_turn_missing');
  assert.equal(result.reason, 'leading_turn_missing');
