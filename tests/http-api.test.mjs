...
test('http-api: catalog service states have stable content-free status mappings', async (t) => {
  ...
  const cases = [
    ...
    ['catalog_import_capacity_required', 409, 'catalog_import_capacity_required'],
    ...
    ['catalog_import_inspection_failed', 500, 'catalog_import_inspection_failed'],
    ['catalog_service_unavailable', 500, 'catalog_service_unavailable'],
...
test('http-api: query returns runId and persists durable run state', async (t) => {
  ...
  assert.equal(persisted.modelIntent, null);
...
test('http-api: query forwards explicit model intent without persisting it as key meta', async (t) => {
  ...
  const controller = {
    runExclusive: async (fn) => await fn(),
    query: async ({ modelIntent, onProgress }) => {
  ...
  assert.equal(persisted.conversationUrl, 'https://chatgpt.com/g/g-p-agentify/c/model-intent-run');
  const projectsPath = path.join(stateDir, 'projects.json');
  const projectText = await fs.readFile(projectsPath, 'utf8').catch(() => '{}');
  assert.equal(JSON.parse(projectText).default?.modelIntent, undefined);
});

test('http-api: explicit model intent fails if controller returns without confirmation provenance', async (t) => {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agentify-http-model-unconfirmed-'));
  const controller = {
    runExclusive: async (fn) => await fn(),
  ...
  const response = await req({
    ...
    method: 'POST',
    pth: '/query',
    body: { prompt: 'must confirm model', source: 'mcp', modelIntent: 'gpt-5.4-pro' }
  });
  assert.equal(response.res.status, 409);
  assert.equal(response.data.error, 'model_intent_activation_failed');
...
test('http-api: tabs/create persists ChatGPT mode intent on keyed tabs', async (t) => {
  ...
  assert.equal(ensuredArgs.modelIntent, undefined);
  const persisted = JSON.parse(await fs.readFile(path.join(stateDir, 'projects.json'), 'utf8'));
  assert.equal(persisted.main.modeIntent, 'thinking');
  assert.equal(persisted.main.modelIntent, undefined);
});

test('http-api: tabs/create rejects sticky model intent defaults', async (t) => {
  const tabs = {
    listTabs: () => [],
    ensureTab: async () => {
      throw new Error('should_not_create_tab');
    },
    createTab: async () => {
      throw new Error('should_not_create_tab');
    },
  ...
  assert.equal(r.data.error, 'model_intent_unsupported');
  assert.equal(r.data.data?.normalizedModelIntent, 'gpt-5.5-pro');
});
